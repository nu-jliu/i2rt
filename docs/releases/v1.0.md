...
- `PassiveEncoderReader` simplified — encoder validation and configurable `range_rad` removed in favour of hardcoded defaults.
- Removed per-frame `timestamp` field from `MotorInfo`; `motor_chain_robot` now uses `time.time()` directly.
